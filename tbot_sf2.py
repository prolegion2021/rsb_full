...
def to_storage():
    b = api.imgsearch(sf2_folder, sf2_arch_icon_img, sf2_arch_icon_res, winname, 0).img_search(sf2_arch_icon_dev)
    if b[0] != -1:
        rtime = random.randint(10, 12)
        x, y = b[0]
        r = random.randint(-2, 2)
        cx, cy = sf2_arch_icon_to_storage
        api.core(x+r, y+r, winname).adb_tap(bswinname, cx, cy, 0, 'tap_swipe')
        time.sleep(rtime)
        all_to_storage()
    else:
...
def all_to_storage():
    b = api.imgsearch(sf2_folder, sf2_bank_img, sf2_bank_res, winname, 0).img_search(sf2_bank_dev)
    if b[0] != -1:
        x, y = b[0]
...
def confirm_destroy():
    global player_pos
    b = api.imgsearch(sf2_folder, sf2_destroy_img, sf2_destroy_res, winname, 0).img_search(sf2_destroy_dev)
    if b[0] != -1:
        x, y = b[0]
        r = random.randint(-2, 2)
...
def drop():
    b = api.imgsearch(sf2_folder, sf2_drop_img, sf2_drop_res, winname, 0).img_search(sf2_drop_dev)
    ...
    elif b[0] != -1 and b[1] == 'drop_scroll.png':
        print('Нашел дроп', b[1])
        r = random.randint(-3, 3)
        rx = random.randint(-490, -450)
        ry = random.randint(-100, 30)
    ...
    elif b[0] != -1 and b[1] == 'drop_tome.png':
        print('Нашел дроп', b[1])
        r = random.randint(-3, 3)
        rx = random.randint(-490, -450)
        ry = random.randint(-100, 30)
...
def ore_mining():
    b = api.imgsearch(sf2_folder, sf2_ore_img, sf2_ore_res, winname, 0).img_search(sf2_ore_dev)
    if in_proc():
        if b[0] != -1:
            x, y = b[0]
            r = random.randint(-2, 2)
...
def in_proc():
    global player_pos
    b = api.imgsearch(sf2_folder, sf2_inproc_img, sf2_inproc_res, winname, 0).img_search(sf2_inproc_dev)
    if b[0] != -1:
        drop()
        return False
    else:
...
def ore_check():
    ...
    i = 0
    while i <= 7:
        b = api.imgsearch(sf2_folder, sf2_invfull_img, sf2_invfull_res, winname, 0).img_search(sf2_invfull_dev)
        time.sleep(1)
        i = i + 1
        if b[0] != -1:
...
def main():
    ...
    print('( Второй этаж - Вторая комната )')
